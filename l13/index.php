...
$absoluteRout = "{$storage}{$rout}";
$absoluteRout = realpath($absoluteRout);
$rout = str_replace($storage, '', $absoluteRout);
...
<main class="container">
    ...
    <div class="bg-light p-5 rounded">
        <form method="post" class="row" action="actions/create-dir.php?rout=<?= $rout?>">
            <div class="col-8">
            <label for="directory_name_input" class="d-none"></label>
            <input type="text"
                   name="dir_name"
    ...
    <ul class="mt-3">
        <?php foreach (getFilesList() as $file): ?>
            <li><a href="index.php?rout=<?= $rout ?>/<?= $file ?>"><?= $file ?></a></li>
        <?php endforeach; ?>
    </ul>
</main>
